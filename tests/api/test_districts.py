...
class TestDistrictsAPI(TestBaseServer):
    ...
    def test_put_districts(self):
        self.assert403("/adele/api/1.0/districts/from-country/1", data={"data": {}},  method="PUT")
        self.assert403("/adele/api/1.0/districts/from-country/1", data={"data": {}},  method="PUT", **STU1_USER)
        self.assert409("/adele/api/1.0/districts/from-country/1", data={"data": [{"id": 100}]},  method="PUT", **ADMIN_USER)

        self.assert200("/adele/api/1.0/districts/from-country/1",
                       data={"data": [{"id": 1, "label": "DistrictTest"}]},
                       method="PUT", **ADMIN_USER)
        r = self.assert200('/adele/api/1.0/districts/1/from-country/1')
        r = json_loads(r.data)["data"]
        self.assertEqual("DistrictTest", r[0]["label"])

        # put conflicting data
        # missing district id
        self.assert409("/adele/api/1.0/districts/from-country/1", data={"data": [{"label": "Desc2"}]}, method="PUT",
                       **ADMIN_USER)

        self.assert409("/adele/api/1.0/districts/from-country/1",
        ...
        self.assert200("/adele/api/1.0/districts/from-country/2",
    ...
    def test_api_post_district(self):
        ...
        r = self.assert200('/adele/api/1.0/districts/500/from-country/1')
        r = json_loads(r.data)["data"]
        ...
        self.assert200("/adele/api/1.0/districts",
                       ...
                       **PROF1_USER)
        self.assert200('/adele/api/1.0/districts/501/from-country/3')
        self.assert200('/adele/api/1.0/districts/502/from-country/2')
